    ...
    <main style={styles.main}>
      <div style={styles.container}>
        <div style={styles.header}>
          ...
          <p style={styles.subtitle}>
            Buat caption media sosial yang engaging dalam hitungan detik
          </p>
        </div>
        ...
        <div style={styles.card}>
          <div style={styles.field}>
            ...
            <input
              style={styles.input}
              placeholder="contoh: Jual sepatu sneakers limited edition"
              value={topik}
            ...
            <div style={{ ...styles.field, flex: 1 }}>
              ...
              <div style={styles.chipGroup}>
                {PLATFORMS.map((p) => (
                  <button
                    key={p.value}
                    style={platform === p.value ? styles.chipActive : styles.chip}
            ...
            <div style={{ ...styles.field, flex: 1 }}>
              ...
              <div style={styles.chipGroup}>
                {TONES.map((t) => (
                  <button
                    key={t.value}
                    style={tone === t.value ? styles.chipActive : styles.chip}
          ...
          <div style={styles.field}>
            <label style={styles.label}>
              Detail tambahan{" "}
              <span style={{ fontWeight: 400, color: "#888" }}>(opsional)</span>
            </label>
            <textarea
              style={styles.textarea}
              placeholder="contoh: warna ada pink, mint, cream — bahan adem, harga 85rb, cocok untuk daily & kondangan"
              value={detail}
          ...
          <button
            style={loading || !topik.trim() ? styles.btnDisabled : styles.btn}
            onClick={generate}
      ...
      <style>{`
        ...
        * { box-sizing: border-box; }
        ...
        input:focus, textarea:focus { outline: none; border-color: #6c63ff !important; }
      `}</style>
    </main>
  );
}
...
const styles: Record<string, React.CSSProperties> = {
  ...
  input: { padding: "10px 14px", borderRadius: 10, border: "1.5px solid #e5e7eb", fontSize: 14, color: "#1a1a2e", background: "#fff", transition: "border-color 0.15s" },
  textarea: { padding: "10px 14px", borderRadius: 10, border: "1.5px solid #e5e7eb", fontSize: 14, color: "#1a1a2e", background: "#fff", resize: "vertical", fontFamily: "inherit", lineHeight: 1.5, transition: "border-color 0.15s" },
  chipGroup: { display: "flex", flexWrap: "wrap", gap: 8 },
  chip: { padding: "6px 14px", borderRadius: 20, border: "1.5px solid #e5e7eb", background: "#fff", fontSize: 13, color: "#6b7280", cursor: "pointer", transition: "all 0.15s", fontWeight: 500 },
  ...
  btn: { padding: "12px 20px", borderRadius: 10, border: "none", background: "#6c63ff", color: "#fff", fontSize: 15, fontWeight: 600, cursor: "pointer", transition: "opacity 0.15s", letterSpacing: "0.01em" },
  btnDisabled: { padding: "12px 20px", borderRadius: 10, border: "none", background: "#c4b5fd", color: "#fff", fontSize: 15, fontWeight: 600, cursor: "not-allowed", letterSpacing: "0.01em" },
